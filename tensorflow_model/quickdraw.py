...
tf.flags.DEFINE_string("data_dir", "tmp/quickdraw", "")
tf.flags.DEFINE_boolean("read_attn", True, "enable attention for reader")
tf.flags.DEFINE_boolean("write_attn",True, "enable attention for writer")
FLAGS = tf.flags.FLAGS

def quickdraw(draw_obj, train_iters, verbose=True):
    if verbose:
        print('Running quickdraw with object %s for %d iters.' % (draw_obj, train_iters))
    ...
    draw_data = np.load('../quickdraw_data/%s.npy' % (draw_obj,)) # [:20000]
    draw_data = draw_data / 255

    # batch_size = 100
    n_training_images = draw_data.shape[0]
    split_size = n_training_images // batch_size

    print('# of training images:', n_training_images)
    print('split size:', split_size)


    def get_next_batch(n=batch_size):
        # for i in range(0, ambulance.shape[0], n):
        k = 0
    ...
    # output_dir = os.path.join(FLAGS.data_dir,"%s/%d/quickdraw_%s_data.npy" % (draw_obj,train_iters,draw_obj))
    output_dir = os.path.join(FLAGS.data_dir,"%s/%d" % (draw_obj,train_iters))
    print(output_dir)

    sess=tf.InteractiveSession()

    tf.train.write_graph(sess.graph_def, output_dir, 'banana.pbtxt', as_text=True)

    #saver = tf.train.Saver(write_version=tf.train.SaverDef.V1) # saves variables learned during training
    saver = tf.train.Saver()
    tf.global_variables_initializer().run()
    # saver.restore(sess, "tmp/quickdraw/quickdraw_ambulance_drawmodel.ckpt.meta") # to restore from model, uncomment this line
    ...
    for xtrain in get_next_batch():
        ...
        try:
            ...
            break
        if i >= train_iters:
            # 10 000 runs into error?
            break

    endTime = time.time()
    elapsedTime = endTime - startTime
    print('Training started at %s, ended at %s, for a total time of %s.' % (str(startTime), str(endTime), str(elapsedTime)))
    ## TRAINING FINISHED ##
    ...
    out_file=os.path.join(output_dir, 'quickdraw_%s_data.npy' % (draw_obj,))
...
if __name__ == '__main__':
    # draw_obj = 'pear'
    # train_iters=25001
    quickdraw(draw_obj='banana', train_iters=101)
else:
    print('Imported script quickdraw from %s' % (os.getcwd(),))
